...
func main() {
	fonts, err := crt.LoadDefaultFaces(crt.GetFontDPI(), 24.0)
	if err != nil {
		panic(err)
	}

	win, _, err := bubbleadapter.Window(1920, 1080, fonts, NewModel(), color.Black)
	if err != nil {
		panic(err)
	}
